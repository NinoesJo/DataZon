...
def login():
    ...
    if form.validate_on_submit():
        user = Account.get_by_auth(form.email.data, form.password.data)
        if user is None:
...
class RegistrationForm(FlaskForm):
    ...
    password2 = PasswordField('Repeat Password', validators=[DataRequired(), Length(max=64), EqualTo('password')])
    is_seller = BooleanField('I want to create a seller account')
...
def register():
    ...
    if form.validate_on_submit():
        if Account.register(form.email.data,
                         form.password.data,
                         form.full_name.data,
                         form.is_seller.data):
